...
$(function () {
  ...
  var getPeriods = toggleVisibility(
    $('.fieldset-periods'),
    $(".fieldset-subjects, .fieldset-slos"),
  ...
  var getPrograms = toggleVisibility(
    $('.fieldset-programs'),
    $('.fieldset-slos'),
    function() { return $period.val() === ""; },
    null,
    function() { return $program.val() !== ""; },
  ...
  var getSLOs = toggleVisibility(
    $('.fieldset-slos'),
    null,
    function() { return (!$program.length && $subject.val() === "") || ($program.length && $program.val() === "") || $year.val() === "" || $period.val() === ""; },
    function() {
      var params = {year:$year.val(), period:$period.val()};
      if ($program.length) params.program = $program.val();
      else params.subject = $subject.val();

      getReportSummary($slos, "psloSummaryData", params);
    }
  );
  ...
  if ($program.length) {
    $period.change(getPrograms);
    $program.change(getSLOs);
  } else {
    $period.change(getSubjects);
    $subject.change(getSLOs);
  }
});