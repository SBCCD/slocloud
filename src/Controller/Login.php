...
class Login extends Base
{
    ...
    public function postLogin()
    {
        ...
        if ($username === null || $password === null) {
            $app->flash('error', "No credentials specified!");
            $app->redirect('login');
        }

        $ldap = $app->ldap();
        $account = $ldap->login($username, $password);
        if ($account === false) {
            $error = "Failed to login. (".$ldap->lastError().")";
            $app->writeError($error . " ($username)");
            $app->flash('error', $error);
            $app->redirect('login'.$app->queryString());
        } elseif ($account['AccountId'] === '') {
            $error = $app->config('no-valid-id-message');
            $app->writeError($error);
            $app->flash('error', $error);
            $app->redirect('login'.$app->queryString());
        }

        if (!$app->isAllowedUser($account['AccountId'])) {
            $error = $app->config('not-allowed-id-message');
            $app->writeError($error);
            $app->flash('error', $error);
            $app->redirect('login'.$app->queryString());
        }

        $app->saveLogin($account['AccountId'], $account['Type']);
